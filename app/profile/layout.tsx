...
export default function layout({
  ...
  // ✅ вот тут, прямо внутри компонента layout
  const handleLogout = () => {
    ...
    <div className="flex gap-2 p-6">
      <aside className="p-5 bg-white rounded-2xl flex flex-col justify-between h-[550px] w-100">
        <div className="flex flex-col gap-2">
          <Link href="/profile">
            <div className={`${path === '/profile' ? 'active' : 'grayscale'} flex gap-2 bg-amber-50 p-3 rounded-2xl`}>
              <Image src={profileLogo} alt="profileLogo" />
              Профиль
            </div>
          </Link>
          <Link href="/profile/ads">
            <div className={`${path === '/profile/ads' ? 'active' : 'grayscale'} flex gap-2 bg-amber-50 p-3 rounded-2xl`}>
              <Image src={bag} alt="bag" />
              Мои обьявления
            </div>
          </Link>
          <Link href="/profile/favorites">
            <div className={`${path === '/profile/favorites' ? 'active' : 'grayscale'} flex gap-2 bg-amber-50 p-3 rounded-2xl`}>
        ...
        {/* ✅ вот тут вызывается handleLogout */}
        <button onClick={handleLogout} className="flex gap-2 text-red-500">
          <Image src={leave} alt="leave" /> Выйти из аккаунта
        </button>
      </aside>
